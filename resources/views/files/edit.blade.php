...
{{-- フォーム --}} 
<form action="{{ url('files/' . $item->id) }}" method="post">
  @csrf
  ...
  <div class="form-group">
    <label for="file_c_memo">{{ trans('validation.attributes.c_memo')}} <span class="sp"><br></span><span style="color:green;">{{ trans('validation.attributes.msg_null_plus') }}</span></label>
    @error('c_memo')
